...
internal static class LocalizationRegistry
{
    ...
    public static Dictionary<string, Dictionary<string, string>> Build()
    {
        return new Dictionary<string, Dictionary<string, string>>
        {
            ...
            [StringKeys.Security.HowItWorksBullet5] = new() { ["en"] = "Update checks send only app version and OS info, including an anonymous install ID — no document data", ["ko"] = "업데이트 확인은 앱 버전과 OS 정보, 익명 설치 ID만 전송합니다 — 문서 데이터 없음", ["fr"] = "Les vérifications envoient uniquement la version et les infos système, y compris un ID d'installation anonyme — aucune donnée documentaire", ["de"] = "Update-Prüfungen senden nur App-Version und OS-Infos inkl. anonymer Installations-ID — keine Dokumentdaten", ["zh"] = "更新检查仅发送应用版本和操作系统信息（含匿名安装ID）——不包含文档数据" },
            [StringKeys.Security.PingTitle] = new() { ["en"] = "Update check", ["ko"] = "업데이트 확인", ["fr"] = "Vérification des mises à jour", ["de"] = "Update-Prüfung", ["zh"] = "更新检查" },
            [StringKeys.Security.PingDescription] = new() { ["en"] = "LocalSynapse sends only app version, system info (OS, language), and an anonymous install ID to check for updates. No files, no search history, no personal data is ever transmitted. You can disable this in Settings.", ["ko"] = "LocalSynapse는 업데이트 확인을 위해 앱 버전, 시스템 정보(OS, 언어), 익명 설치 ID만 전송합니다. 파일, 검색 기록, 개인 정보는 전송되지 않습니다. 설정에서 비활성화할 수 있습니다.", ["fr"] = "LocalSynapse n'envoie que la version de l'application et les informations système (OS, langue) pour vérifier les mises à jour. Aucun fichier, historique de recherche ou donnée personnelle n'est jamais transmis. Vous pouvez désactiver cette fonction dans les paramètres.", ["de"] = "LocalSynapse sendet nur die App-Version und Systeminformationen (OS, Sprache) zur Update-Prüfung. Keine Dateien, kein Suchverlauf und keine persönlichen Daten werden übertragen. Sie können dies in den Einstellungen deaktivieren.", ["zh"] = "LocalSynapse仅发送应用版本和系统信息（操作系统、语言）以检查更新。不会传输任何文件、搜索历史或个人数据。您可以在设置中禁用此功能。" },
            ...
            [StringKeys.Security.Sends.SendsLabel] = new() { ["en"] = "Sends: Anonymous version ping", ["ko"] = "보냄: 익명 버전 핑", ["fr"] = "Envoie : ping de version anonyme", ["de"] = "Sendet: Anonymer Versions-Ping", ["zh"] = "发送：匿名版本 ping" },
            ...
            [StringKeys.Security.Sends.ExpandTitle] = new() { ["en"] = "What's in the anonymous ping?", ["ko"] = "익명 핑에 어떤 정보가 들어 있나요?", ["fr"] = "Que contient le ping anonyme ?", ["de"] = "Was enthält der anonyme Ping?", ["zh"] = "匿名 ping 包含什么？" },
            [StringKeys.Security.Sends.ExpandBody] = new() { ["en"] = "Once per day, LocalSynapse sends a small anonymous record: Anonymous install ID (random UUID), App version, OS, and Locale. This cannot be linked to you, your files, or your activity. Your documents are never sent.", ["ko"] = "하루 1회 익명 레코드를 전송합니다: 익명 설치 ID (랜덤 UUID), 앱 버전, OS, 로캘. 이 정보는 사용자, 파일, 활동과 연결할 수 없습니다. 문서는 절대 전송되지 않습니다.", ["fr"] = "Une fois par jour, LocalSynapse envoie un petit enregistrement anonyme : ID d'installation anonyme (UUID aléatoire), version de l'app, OS et locale. Cela ne peut pas être lié à vous, vos fichiers ou votre activité. Vos documents ne sont jamais envoyés.", ["de"] = "Einmal täglich sendet LocalSynapse einen kleinen anonymen Datensatz: Anonyme Installations-ID (zufällige UUID), App-Version, OS und Locale. Dies kann nicht mit Ihnen, Ihren Dateien oder Ihrer Aktivität verknüpft werden. Ihre Dokumente werden nie gesendet.", ["zh"] = "每天一次，LocalSynapse 发送一小段匿名记录：匿名安装 ID（随机 UUID）、应用版本、操作系统和语言。这些无法与您、您的文件或活动关联。您的文档永远不会被发送。" },
